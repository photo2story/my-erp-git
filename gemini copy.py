...
from Get_data import get_post_market_prices  # 통일된 함수 가져오기
...
RELATIVE_DIVERGENCE_CSV = os.path.join(STATIC_IMAGES_PATH, "results_relative_divergence.csv")
    ...
    try:
       ...
       1) 제공된 자료의 수익율(rate)와 S&P 500(VOO)의 수익율(rate_vs)과 비교해서 이격된 정도를 알려줘 (간단하게 자료 맨마지막날의 누적수익율차이):
        ...
        3) 제공된 자료의 RSI, PPO 인덱스 지표와 Delta_Previous_Relative_Divergence,Expected_Return 을 분석해줘 (간단하게):
           ...
           기대수익(%) = {Expected_Return}, 현 시점부터 장기적으로(2년이상)적립 투자할 경우 예상되는 S&P 500대비 초과 수익률
        """
        ...
        4) Recent Earnings Analysis:
           ...
           Please analyze:
           - Latest quarterly results comparison with previous quarters
           - Revenue growth trends
           - EPS trends and significant changes
           - Highlight any notable beats or misses vs expectations
           - Only focus on actual reported numbers shown in the table
           - Do not speculate about missing data or incomplete context
        """

        # 재무정보가 있는 경우에만 추가
        if has_financials and not is_crypto:
            prompt_voo += f"""
        5) 재무정보 = {financials_summary}  표를 보여주고 제공된 분기별 데이터를 분석해줘
        """

        # 뉴스 섹션 추가 (암호화폐와 주식에 따라 다른 내용)
        if is_crypto:
            prompt_voo += """
        6) 뉴스와 최근 이슈:
           - 최근 2일 이내의 주요 암호화폐 시장 동향
           - 규제 관련 업데이트
           - 기술적 발전 사항
           - 시장 심리 분석
        """
