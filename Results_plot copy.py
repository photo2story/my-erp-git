...
async def plot_comparison_results(ticker, start_date, end_date, simplified_df=None, combined_df=None):

    stock2 = 'VOO'
    fig, ax2 = plt.subplots(figsize=(8, 6))

    # ✅ simplified_df는 항상 별도로 로드
    simplified_path = os.path.join(config.STATIC_IMAGES_PATH, f'result_{ticker}.csv')
    try:
        simplified_df = pd.read_csv(simplified_path, parse_dates=['Date'], index_col='Date')
        print(f"Loaded simplified data from {simplified_path}")
    except Exception as e:
        print(f"Error loading {simplified_path}: {e}")
        return

    # ✅ 날짜 필터링
    if start_date is None:
        start_date = simplified_df.index.min()
    if end_date is None:
        end_date = simplified_df.index.max()

    simplified_df = simplified_df.loc[start_date:end_date]

    # ✅ 수익률 컬럼 지정
    ticker_col = f'rate_{ticker}_5D'
    voo_col = 'rate_VOO_20D'

    if ticker_col not in simplified_df.columns or voo_col not in simplified_df.columns:
        print(f"[ERROR] Required columns missing: {ticker_col}, {voo_col}")
        return

    # ✅ 그래프 그리기
    ax2.plot(simplified_df.index, simplified_df[ticker_col], label=f'{ticker} Monthly Return')
    ax2.plot(simplified_df.index, simplified_df[voo_col], label='VOO Monthly Return')
    plt.ylabel('Total Return (%)')
    plt.legend(loc='upper left')
    ax_right = ax2.twinx()
    ax_right.set_ylim(ax2.get_ylim())

    # ✅ 지표용 데이터프레임 (combined_df 없으면 로딩)
    if combined_df is None:
        ...
        try:
            ...
            print(f"Loaded combined_df from {csv_path}")
        except Exception as e:
    ...
    mri = 0.0
    hybrid_signal = "N/A"
    total_rate = simplified_df[ticker_col].iloc[-1] if not simplified_df.empty else 0
    voo_rate = simplified_df[voo_col].iloc[-1] if not simplified_df.empty else 0
    ...
    try:
        ...
        cash_signal_row = df_voo[df_voo['signal'].astype(str).str.startswith('cash_')]
        if not cash_signal_row.empty:
            raw_signal = cash_signal_row.iloc[-1]['signal']
            parts = raw_signal.split('_', 2)
            hybrid_signal = f"{parts[0]}_{parts[1]}" if len(parts) >= 2 else "N/A"
    except Exception as e:
        print(f"Error processing hybrid signal from combined_df: {e}")
        hybrid_signal = "N/A"
    ...
    try:
        ...
        max_divergence = min_divergence = current_divergence = relative_divergence = expected_return = recent_signal = 0.0
